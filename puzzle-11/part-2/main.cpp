...
long long getLengths(std::vector<std::pair<int, int>> &, std::vector<std::vector<char>> &);
...
void map(std::vector<std::pair<int, int>> &gmap, std::vector<std::vector<char>> &g){
    ...
    std::cout << "Number of galaxies: " << gmap.size() << " Number of pairs: " << (gmap.size()*(gmap.size()-1)/2)<< std::endl;
    return;
}

long long getLengths(std::vector<std::pair<int, int>> &gmap, std::vector<std::vector<char>> & g){
    int s_count = 0;
    ...
    long long result = 0; int temp_r = 0; int temp_c = 0;
    for(int root = 0; root < gmap.size() - 1; root++){
        for(int pair = root + 1; pair < gmap.size(); pair++){
            temp_r = (rows[gmap[pair].first] - rows[gmap[root].first])*EXPANSE - 1*(rows[gmap[pair].first] - rows[gmap[root].first]);
            temp_c = (cols[gmap[pair].second] - cols[gmap[root].second])*EXPANSE -1*(cols[gmap[pair].second] - cols[gmap[root].second]);
            result += abs((gmap[pair].first + temp_r - gmap[root].first)) + abs((gmap[pair].second + temp_c - gmap[root].second));
